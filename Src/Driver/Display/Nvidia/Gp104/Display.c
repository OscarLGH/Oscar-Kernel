...
static RETURN_STATUS Gp104DisplayDmaChanInit(DISPLAY_ENGINE * Engine)
{
	...
	NvMmioMask(Gpu, 0x6100a0, 0x00000001, 0x00000001);

	/* initialise channel for dma command submission */
	//NvMmioWr32(Gpu, 0x611494, (UINT32)Engine->CoreChannel->RunList[0]->Addr);
	NvMmioWr32(Gpu, 0x611498, 0x00010000);
	NvMmioWr32(Gpu, 0x61149c, 0x00000001);
	...
	while (WaitDelay--)
	{
		if (!NvMmioRd32(Gpu, 0x610490) & 0x80000000)
			break;
	...
	if (WaitDelay == 0)
	{
		KDEBUG("Core init: %08x\n", NvMmioRd32(Gpu, 0x610490));
...
RETURN_STATUS Gp104DisplayInit(DISPLAY_ENGINE * Engine)
{
	...
	KDEBUG("NVIDIA:Dcb:Version = %x, Head = %x, Cnt = %x, Len = %x\n", Ver, Head, Cnt, Len);
	...
	for (i = 0; i < Head; i++)
	{
		...
		KDEBUG("NVIDIA:Total = %x, Blanke = %x, Blanks = %x, VLine = %x, HLine = %x\n", Total, Blanke, Blanks, VLine, HLine);
	}
