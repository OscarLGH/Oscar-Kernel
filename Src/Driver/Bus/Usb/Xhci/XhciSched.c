  ...
  )
{
  ...
  for (Index = 0; Index < Xhc->EventRing.TrbNumber; Index++) {
    ...
    //
    PhyAddr = (VOID *)(EvtTrb->TRBPtrLo | LShiftU64 ((UINT64) EvtTrb->TRBPtrHi, 32));
  ...
  )
{
  ...
  for (EpIndex = 0; EpIndex < NumEp; EpIndex++) {
    ...
    PhyAddr &= ~((VOID *)0x0F);
    PhyAddr |= (VOID *)((TRANSFER_RING *)(UINTN)Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci-1])->RingPCS;
