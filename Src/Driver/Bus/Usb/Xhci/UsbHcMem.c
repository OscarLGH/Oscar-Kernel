  ...
  )
{
  ...
  Block = KMalloc (sizeof (USBHC_MEM_BLOCK));
  if (Block == NULL) {
    return NULL;
  }
  ...
  //
  ASSERT (USBHC_MEM_UNIT * 8 <= 0x1000);

  Block->BufLen   = Pages * 0x1000;
  Block->BitsLen  = Block->BufLen / (USBHC_MEM_UNIT * 8);
  Block->Bits     = KMalloc (Block->BitsLen);
  ...
  )
{
  ...
  PhyAddr = (VOID *)(UINTN) (Block->Buf + Offset);
  return PhyAddr;
}
  ...
  )
{
  ...
  HostAddr = (VOID *)(UINTN) (Block->BufHost + Offset);
  return HostAddr;
}
  ...
  )
{
  ...
  Pool = KMalloc (sizeof (USBHC_MEM_POOL));
  ...
  )
{
  ...
  if (NewBlock == NULL) {
    KDEBUG ("UsbHcAllocateMem: failed to allocate block\n");
    return NULL;
  }
  ...
  )
{
  ...
  if (Alignment > 0x1000) {
    ...
    RealPages      = Pages + (Alignment * 0x1000);
    //
    // Make sure that Pages plus EFI_SIZE_TO_PAGES (Alignment) does not overflow.
    //
    ASSERT (RealPages > Pages);
 
	Memory = KMalloc(Pages * 0x1000);
   
    AlignedMemory  = ((UINTN) Memory + AlignmentMask) & ~AlignmentMask;
    UnalignedPages = (AlignedMemory - (UINTN) Memory) * 0x1000;
    if (UnalignedPages > 0) {
      //
    ...
    Memory         = (VOID *)(UINTN)(AlignedMemory + (Pages) * 0x1000);
