...
static long long KeyboardSpinLock = 0;

static void KeyboardHandler(void *);
static UINT8 GetByteFromKBuf();
...
void KeyboardInit()
{
	...
	IrqRegister(0x21, KeyboardHandler, NULL);
	EnableIrqn(0x1);

	SetLEDs();

}
void KeyboardHandler(void * Dev)
{
	//GetSpinLock(&KeyboardSpinLock);
	UINT8 scan_code = GetKeyboardData();
	//KDEBUG("%x",scan_code);
	...
	//ReleaseSpinLock(&KeyboardSpinLock);
}
UINT32 GetKey(void)
{
	UINT8 scan_code;
	char output[2];
	int make;      //true:makecode false:breakcode
	UINT32 key = 0;
	UINT32 *keyrow;
	...
	{
		code_with_E0 = 0;
		scan_code = GetByteFromKBuf();
		//printk("  302:%x  ,",scan_code);
		if(scan_code == 0xe1)
		{
			int i;
			UINT8 pausebrk_scode[] = {0xe1,0xd1,0x45,0xe1,0x9d,0xc5};
			int is_pausebreak = 1;
			for(i = 1; i < 6;i++)
				if(GetByteFromKBuf()!= pausebrk_scode[i])
				{
					is_pausebreak = 0;
					break;
				}
		...
		else if(scan_code == 0xe0)
		{
			...
			/*以下4个if为处理NumLock+方向键*/
			if(scan_code == 0x2A)
			{
				if(GetByteFromKBuf() == 0xe0)
				{
					if(GetByteFromKBuf() == 0x48)
					{
						key = UP;
						make = 1;
					}
					if(GetByteFromKBuf() == 0x50)
					{
						key = DOWN;
						make = 1;
					}
					if(GetByteFromKBuf() == 0x4B)
					{
						key = LEFT;
						make = 1;
					}
					if(GetByteFromKBuf() == 0x4D)
					{
						key = RIGHT;
						make = 1;
					}
		...
		if((key != PAUSEBREAK) && (key != PRINTSCREEN))
		{
			...
			keyrow = &keymap[(scan_code&0x7f)*MAP_COLS];  //取所在行，&0x7f 可以将breakcode转换为makecode,同时避免数组越界
			column = 0;
...
void SetLEDs()
{
	...
	//while(In8(0x64)&0x02);		//等待8042输入缓冲区为空
	//Out8(0x60,0xED);
	//Out8(0x60,(1<<2) | (1<<1) | 1);
	//Delay(0xFFFFF);
	//Out8(0x60,0xED);
	//Out8(0x60,0);
	/*
	KB_DATA		0x60	  I/O port for keyboard data
	Read : Read Output Buffer
	Write: Write Input Buffer(8042 Data&8048 Command) 
	KB_CMD		0x64	  I/O port for keyboard command
	Read : Read Status Register
	Write: Write Input Buffer(8042 Command) 
	LED_CODE	0xED
	KB_ACK		0xFA
	*/
	//while(In8(0x60)!= 0xFA);	//等待响应
	//while(In8(0x64)&0x02);	//等待8042输入缓冲区为空
	//Out8(0x60,leds);
	//while(In8(0x60)!= 0xFA);	//等待响应
}
