...
{
	u32 bar_val = 0;
	pci_read_config_dword(pdev, PCI_CONF_BAR0 + bar * 4, &bar_val);
	if (bar_val & BIT1) {
		return 0xffff;
	}
	return bar_val & 0xf;
...
u64 pci_get_oprombar_base(struct pci_dev *pdev)
{
	u32 bar = 0;
	if (pdev->type & 0x1 == 0) {
		pci_read_config_dword(pdev, PCI_CONF_TYPE0_EXPANSION_ROM_BAR, &bar);
	} else {
		pci_read_config_dword(pdev, PCI_CONF_TYPE1_EXPANSION_ROM_BAR, &bar);
	}
	return (bar & (~0xf));
}
...
u64 pci_get_oprombar_size(struct pci_dev *pdev)
{
	...
	if ((pdev->type & 0x1) == 0) {
		pci_read_config_dword(pdev, PCI_CONF_TYPE0_EXPANSION_ROM_BAR, &ori_bar);
		pci_write_config_dword(pdev, PCI_CONF_TYPE0_EXPANSION_ROM_BAR, &all_ones);
		pci_read_config_dword(pdev, PCI_CONF_TYPE0_EXPANSION_ROM_BAR, &size);
		pci_write_config_dword(pdev, PCI_CONF_TYPE0_EXPANSION_ROM_BAR, &ori_bar);
	} else {
		pci_read_config_dword(pdev, PCI_CONF_TYPE1_EXPANSION_ROM_BAR, &ori_bar);
		pci_write_config_dword(pdev, PCI_CONF_TYPE1_EXPANSION_ROM_BAR, &all_ones);
		pci_read_config_dword(pdev, PCI_CONF_TYPE1_EXPANSION_ROM_BAR, &size);
		pci_write_config_dword(pdev, PCI_CONF_TYPE1_EXPANSION_ROM_BAR, &ori_bar);
	}
...
void pci_device_init(struct pci_dev *pdev)
{
	...
	pci_read_config_byte(pdev, PCI_CONF_HEADER_TYPE, &type);
	pci_read_config_word(pdev, PCI_CONF_VENDORID, &vid);
	pci_read_config_word(pdev, PCI_CONF_DEVICEID, &did);
	pci_read_config_byte(pdev, PCI_CONF_BASECLASSCODE, &class_code[0]);
	pci_read_config_byte(pdev, PCI_CONF_SUBCLASSCODE, &class_code[1]);
	pci_read_config_byte(pdev, PCI_CONF_INTERFACECODE, &class_code[2]);
...
void pci_device_print()
{
	...
	list_for_each_entry(pdev, &pci_device_list, list) {
		...
		if ((pdev->type & 0x1) == 0) {
			...
			pci_read_config_byte(pdev, PCI_CONF_TYPE1_PRIMARY_BUS_NUM, &primary_bus);
			pci_read_config_byte(pdev, PCI_CONF_TYPE1_SECONDARY_BUS_NUM, &secondary_bus);
			pci_read_config_byte(pdev, PCI_CONF_TYPE1_SUBORDINATE_BUS_NUM, &subordinate_bus);
			printk("    Primary bus:%d Secondary bus:%d Subordinate bus:%d\n",
				primary_bus,
				secondary_bus,
				subordinate_bus
			);
			pci_read_config_byte(pdev, PCI_CONF_TYPE1_IO_BASE, &tmp0);
			pci_read_config_word(pdev, PCI_CONF_TYPE1_IO_BASE_UPPER16, &tmp2);
			io_base = ((tmp0 >> 4) << 12) + (tmp2 << 16);
			pci_read_config_byte(pdev, PCI_CONF_TYPE1_IO_LIMIT, &tmp0);
			pci_read_config_word(pdev, PCI_CONF_TYPE1_IO_LIMIT_UPPER16, &tmp2);
			io_limit = ((tmp0 >> 4) << 12) + (tmp2 << 16) + 0xfff;

			pci_read_config_word(pdev, PCI_CONF_TYPE1_MEMORY_BASE, &tmp2);
			pci_read_config_word(pdev, PCI_CONF_TYPE1_MEMORY_LIMIT, &tmp3);
			memory_base = (((u64)tmp2 >> 4) << 20);
			memory_limit = (((u64)tmp3 >> 4) << 20) + 0xfffff;

			pci_read_config_word(pdev, PCI_CONF_TYPE1_PREFETCHABLE_MEMORY_BASE, &tmp2);
			pci_read_config_dword(pdev, PCI_CONF_TYPE1_PREFETCHABLE_BASE_UPPER32, &tmp4);
			prefetchable_memory_base = (((u64)tmp2 >> 4) << 20) + ((u64)tmp4 << 32);
			pci_read_config_word(pdev, PCI_CONF_TYPE1_PREFETCHABLE_MEMORY_LIMIT, &tmp2);
			pci_read_config_dword(pdev, PCI_CONF_TYPE1_PREFETCHABLE_LIMIT_UPPER32, &tmp4);
